...
# $Id: DB.pm,v 1.17 2002/07/01 06:55:55 gonter Exp $
#
...
sub saveDB
{
  ...
  for ($type= 0; $type < 32; $type++)
  {
    ...
    for ($idx= 0; $idx <= $#$Data; $idx++)
    {
      ...
      # paulocustodio@yahoo.com : done T2D, TEST: note records may be blank
      # When database records with notes are deleted, deleted notes
      # have to be marked as garbage in lookup table (status = 0xC0) instead
      # of default (status = 0)
      if (defined ($rec))
      ...
      { # empty record, set up an entry for the lookup table
        ...
        $rec=
        {
          'off'     => 0,
          'length'  => 0,
          'flags'   => 0xC0,    # Garbage record
          'filters' => 0xFFFE,  # Filter flags
        };
      }
  ...
  local *FO;
  open (FO, ">$fnmo") || die;
  binmode (FO); # MS-DOS systems need this, T2D: how about Mac?
...
=head1 SYNOPSIS
  ...
  Diagnostics and Debugging functions:
    ...
    get_field_type ($id)                return field type info about $id
    get_field_type_table                return field type table
