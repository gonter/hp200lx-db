...
# $Id: vpt.pm,v 1.7 2001/07/01 23:46:23 gonter Exp $
#
...
sub pack_viewpt_table
{
  ...
  # view point table is not -1 terminated, only empty table has -1
  if (@$tbl) {
      $def = pack('v*', @$tbl);
  }
  else {
  ...
  for ($i= 0; $i <= $cnt; $i++)
  {
    ...
    { # SSL was defined
      ...
      foreach $SSL (@SSL)
      {
        ...
        elsif ($op == 0x0005) { push (@ST, pop (@ST) <= pop (@ST)); } # TEST
        elsif ($op == 0x0006) { push (@ST, pop (@ST) <  pop (@ST)); } # TEST
        elsif ($op == 0x0007) { push (@ST, pop (@ST) >= pop (@ST)); } # TEST
        elsif ($op == 0x0008) { push (@ST, pop (@ST) >  pop (@ST)); } # TEST
        elsif ($op == 0x0009) { push (@ST, pop (@ST) != pop (@ST)); }
        #  elsif ($op == 0x000A) "IN_TOKEN" symbol="->", what's that??
        elsif ($op == 0x000B)
        {
          $x= pop (@ST);
          $y= pop (@ST);
