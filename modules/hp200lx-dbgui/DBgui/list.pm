...
# $Id: list.pm,v 1.1 2001/07/19 14:44:15 gonter Exp $
#
...
sub new
{
  ...
  my $title= shift;     # title used for the X window frame
  my %pars= @_;
  ...
  unless (defined ($top= $pars{top}))
  {
    ...
    $mb_f->command (-label => 'Hide', -command => sub {$top->withdraw ();});
        # T2D: do not withdraw if there is nothing else left on the screen!
    $mb_f->command (-label => 'Save', -command => sub {$DBgui->do_save ();});
    $mb_f->command (-label => 'Save+Exit', -command => sub {$DBgui->do_save_and_exit ();});
    $mb_f->command (-label => 'Exit (no Save!)', -command => sub {exit;});
...
{
  my $DBlist= shift;
  print "List View Add Record not yet implemented!\n";
  my $gui= $DBlist->{DBgui};

  my $card= $gui->show_card (0);
  $card->add_record ();
}
