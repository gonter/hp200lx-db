...
#
# Graphical user interface for HP 200 LX DBs implemented in Tk
#
# T2D:
...
# $Id: DBgui.pm,v 1.1 2001/07/19 14:44:14 gonter Exp $
#
...
{
  my $class= shift;
  my $db=    shift;     # DB object to bind
  my $title= shift;     # Title used for X frame
  my %pars=  @_;        # optional additional parameters
...
{
  my $DBgui= shift;
  my $name= shift;      # new name or empty to save under original name

  my $db= $DBgui->{db};
  
  print ">>> do_save: update: ", $db->{update}, "\n";
  $db->saveDB ($name);
}

# ----------------------------------------------------------------------------
sub do_save_and_exit
{
  my $DBgui= shift;
  my $name= shift;      # new name or empty to save under original name
  ...
  print ">>> do_exit: update: ", $db->{update}, "\n";
  if ($db->{update})
  {
    $db->saveDB ($name);
  }
